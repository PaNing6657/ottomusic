...
- `invalid_parameter`: 参数格式错误（如 `channel_id` 非数字）
- `resource_not_found`: 资源不存在（如指定的频道不存在或已删除）
...
|-------------|------|----------------|
| `200` | 请求成功 | — |
| `400` | 参数错误或校验失败 | `missing_argument`、`invalid_parameter`、`resource_not_found`、`error_pw` 等 |
| `401` | 认证失败或 Token 无效 | `error_password`、`error_token`、`Token required` |
| `404` | 接口路径不存在 | `Not found` |
| `405` | HTTP 方法不允许 | `Method not allowed` |
| `429` | 请求频率过高 | `too_many_requests` |
...
- `avatar_url`: 用户头像URL
- `cover_url`: 旧版通用封面 URL（旧客户端）
- `cover_h_url`: 横版个人封面 URL；未设置时为默认横版封面图，不回退旧 `cover_url`
- `cover_v_url`: 竖版个人封面 URL；未设置时回退为 `cover_url`
...
**请求参数** (Body):
- `email` (string, 必需): 用户邮箱（支持 QQ / 163 / 126 / Sina 邮箱，规则见下方说明）
...
- `email_exist`: 邮箱已被注册
- `error_email`: 邮箱格式错误（域名不在允许列表，或 163/126/sina 本地部分格式不合法）
- `error_qq_email`: QQ 邮箱格式错误（必须是 5-12 位纯数字 @qq.com，首位不能为 0）
...
**请求参数** (Body):
- `email` (string, 必需): 用户邮箱（支持 QQ / 163 / 126 / Sina 邮箱，规则见下方说明）
- `channel_id` (int, 可选): 频道号。若携带，邮件主题中的 `OTTOhub` 会替换为该频道的名称（`channel_title`）
...
**错误码**:
- `missing_argument`: 缺少必需参数
- `invalid_parameter`: `channel_id` 格式非法（非数字）
- `resource_not_found`: 指定的频道不存在或已删除
- `email_exist`: 邮箱已被注册
- `error_email`: 邮箱格式错误（域名不在允许列表，或 163/126/sina 本地部分格式不合法）
- `error_qq_email`: QQ 邮箱格式错误（必须是 5-12 位纯数字 @qq.com，首位不能为 0）
...
**限制说明**:
- 同一账号10分钟内只能发送一次验证码
- 验证码为6位数字，通过邮件发送（腾讯云 SES）
- 未传 `channel_id` 时，邮件主题为「来自OTTOhub的邀请函」
- 传入有效 `channel_id` 时，邮件主题为「来自{频道名称}的邀请函」（将 `OTTOhub` 替换为频道名称）
...
- 同一账号10分钟内只能发送一次验证码
- 验证码为6位数字，通过邮件发送（腾讯云 SES）
- 未传 `channel_id` 时，邮件主题为「OTTOhub密码找回」
- 传入有效 `channel_id` 时，邮件主题为「{频道名称}密码找回」（将 `OTTOhub` 替换为频道名称）
- 验证码有效期为一定时间（具体由系统配置决定）
...
1. **发送注册验证码**
   ...
   ```
   POST /api/auth/register/verification-code
   Body: { "email": "123456789@qq.com", "channel_id": 10001 }
   ```
...
4. **邮箱限制**:
   - 支持 QQ、163、126、Sina 邮箱注册
   - QQ 邮箱：5-12 位纯数字 @qq.com（首位不能为 0）
   - 163 / 126 / sina 邮箱：字母或数字开头，可含 `.`、`_`、`-`，总长不超过 64 字符
